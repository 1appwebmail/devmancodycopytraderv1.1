...
export function decodeTransaction(info: SubscribeUpdateTransactionInfo, slot: number): DecodedTx {
  ...
  for (const inner of meta.innerInstructions ?? []) {
    for (const ix of inner.instructions) {
      // One malformed/unexpected instruction must never take down decoding of the REST of this
      // transaction's inner instructions — this loop is synchronous, so an uncaught throw here
      // used to abort processing of every remaining instruction in the transaction, silently
      // dropping any legitimate trade/create/migration that happened to come after the bad one in
      // the same tx (a real gap found live: an unregistered event schema threw and this was never
      // caught anywhere up the call chain, becoming an unhandled promise rejection with the whole
      // transaction's events lost, not just the one bad instruction's).
      try {
        ...
        console.error(`decodeTransaction: failed to decode one instruction in tx ${signature}, skipping just that instruction:`, err);
      }
    }
  }
